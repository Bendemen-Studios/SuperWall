...
public sealed class PolicySyncService : BackgroundService
{
    ...
    private readonly SearchHistoryCollector _history = new(); private BlockProxy? _proxy; private LocalControlServer? _local; private PortableBrowserGuard? _portableBrowsers; private AppGuard? _appGuard; private AutoUpdater? _updater; private long _lastAppliedPolicyVersion = -1;
    ...
    { if (_revoked) return; if (_local is null) { _local = new LocalControlServer(() => _policy, ApplyPolicy); _local.Start(); } ApplyPortableBrowserPolicy(); ApplyAppPolicy(); }
    private void ApplyPortableBrowserPolicy()
    { if (_revoked || !_policy.BlockPortableBrowsers) { _portableBrowsers?.Dispose(); _portableBrowsers = null; return; } if (_portableBrowsers is null) { _portableBrowsers = new PortableBrowserGuard(); _portableBrowsers.Start(); } }
    private void ApplyAppPolicy()
    { if (_revoked) { _appGuard?.Dispose(); _appGuard = null; return; } _appGuard ??= new AppGuard(); _appGuard.Apply(_policy); }
    ...
    {
        if (string.IsNullOrWhiteSpace(_dashboard)) return; if (!Uri.TryCreate(_dashboard, UriKind.Absolute, out var baseUri) || !string.Equals(baseUri.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase)) return;
        try
        {
            if (_revoked) { if (!HasEnrollmentKey()) return; _agentToken = ""; }
            ...
            if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized) { ClearRevokedState(); return; }
            if (!response.IsSuccessStatusCode) return; var envelope = await response.Content.ReadFromJsonAsync<PolicyEnvelope>(cancellationToken: ct); if (envelope is null || envelope.Policy.Version < _policy.Version) return;
            _policy = envelope.Policy; SavePolicy(); ApplyPolicy();
            await UploadInstalledApplications(ct);
            foreach (var command in envelope.Commands) if (command.Type.Equals("request_history", StringComparison.OrdinalIgnoreCase)) await UploadHistory(command.Id, ct);
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; } catch { }
    }
    private async Task UploadInstalledApplications(CancellationToken ct)
    {
        ...
        request.Headers.Add("X-SuperWall-Agent", _agentToken); using var response = await _http.SendAsync(request, ct); _ = response.IsSuccessStatusCode;
    }
    private bool HasEnrollmentKey() => !string.IsNullOrWhiteSpace(LoadEnrollmentKey());
    private void ClearRevokedState()
    { _revoked = true; _agentToken = ""; LocalSecrets.Delete("agent-token"); _proxy?.Dispose(); _proxy = null; BrowserPolicy.ClearEnforcement(); DownloadGuard.SetEnabled(false, _policy); _local?.Dispose(); _local = null; _portableBrowsers?.Dispose(); _portableBrowsers = null; _appGuard?.Dispose(); _appGuard = null; }
    private async Task Enroll(CancellationToken ct)
    { var enrollmentKey = LoadEnrollmentKey(); if (string.IsNullOrWhiteSpace(enrollmentKey)) return; using var request = new HttpRequestMessage(HttpMethod.Post, $"{_dashboard.TrimEnd('/')}/api/enroll/{Uri.EscapeDataString(_deviceId)}"); request.Headers.Add("X-SuperWall-Enrollment", enrollmentKey); request.Content = JsonContent.Create(new { computerName = Environment.MachineName, osVersion = Environment.OSVersion.VersionString }); using var response = await _http.SendAsync(request, ct); if (!response.IsSuccessStatusCode) return; var result = await response.Content.ReadFromJsonAsync<EnrollResponse>(cancellationToken: ct); if (result is null || string.IsNullOrWhiteSpace(result.AgentToken)) return; if (!LocalSecrets.Save("agent-token", result.AgentToken)) return; _agentToken = result.AgentToken; _policy = result.Policy ?? new(); SavePolicy(); ConsumeEnrollmentKey(); _revoked = false; ApplyPolicy(); StartLocalServices(); }
    private void SavePolicy() { try { var path = Path.Combine(_stateDir, "policy.json"); var temp = path + ".tmp"; File.WriteAllText(temp, JsonSerializer.Serialize(_policy, new JsonSerializerOptions { WriteIndented = true })); File.Replace(temp, path, null, true); } catch { } }
    private void ApplyPolicy()
    { if (_revoked) return; var policyChanged = _lastAppliedPolicyVersion != _policy.Version; BrowserPolicy.Apply(_policy); if (_policy.UrlBlockingEnabled) { if (_proxy is null) { var candidate = new BlockProxy(host => PolicyRules.IsDomainBlocked(_policy, host)); try { candidate.Start(); _proxy = candidate; } catch { candidate.Dispose(); } } } else { _proxy?.Dispose(); _proxy = null; } DownloadGuard.SetEnabled(_policy.DownloadsBlocked, _policy); ApplyPortableBrowserPolicy(); ApplyAppPolicy(); _lastAppliedPolicyVersion = _policy.Version; if (policyChanged) RestartManagedBrowsers(); }
    ...
    { if (!_policy.SearchHistoryEnabled || string.IsNullOrWhiteSpace(_dashboard) || string.IsNullOrWhiteSpace(_agentToken) || string.IsNullOrWhiteSpace(requestId)) return; var records = _history.Collect(_policy.SearchHistoryRetentionDays); var upload = new HistoryUpload { DeviceId = _deviceId, RequestId = requestId, Records = records }; using var request = new HttpRequestMessage(HttpMethod.Post, $"{_dashboard.TrimEnd('/')}/api/agent/{Uri.EscapeDataString(_deviceId)}/history") { Content = JsonContent.Create(upload) }; request.Headers.Add("X-SuperWall-Agent", _agentToken); using var response = await _http.SendAsync(request, ct); if (!response.IsSuccessStatusCode) return; }
