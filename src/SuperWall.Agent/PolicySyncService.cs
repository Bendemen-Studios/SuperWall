...
public sealed class PolicySyncService : BackgroundService
{
    ...
    private async Task SyncOnce(CancellationToken ct)
    {
        ...
        try
        {
            ...
            if(response.StatusCode==System.Net.HttpStatusCode.Unauthorized){ClearRevokedState();return;}if(!response.IsSuccessStatusCode)return;var envelope=await response.Content.ReadFromJsonAsync<PolicyEnvelope>(cancellationToken:ct);if(envelope is null||envelope.Policy.Version<_policy.Version)return;_policy=envelope.Policy;SavePolicy();ApplyPolicy();
