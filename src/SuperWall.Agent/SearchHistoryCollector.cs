...
public sealed class SearchHistoryCollector
{
    public List<HistoryRecord> Collect(int retentionDays)
    {
        ...
        foreach (var profile in Directory.EnumerateDirectories(Path.Combine(Environment.GetEnvironmentVariable("SystemDrive") ?? "C:", "Users")))
        {
    ...
    private static IEnumerable<HistoryRecord> ReadChromium(string db, string browser, DateTimeOffset since)
    {
        ...
        if (temp is null) yield break;
        try
        {
            using var con = new SqliteConnection($"Data Source={temp}"); con.Open();
            ...
            while (r.Read())
            {
                ...
                yield return new HistoryRecord { Browser = browser, Url = r.GetString(0), Title = r.IsDBNull(1) ? "" : r.GetString(1), VisitedUtc = DateTimeOffset.UnixEpoch.AddTicks(micros * 10) };
            }
        }
        catch { yield break; }
    ...
    private static IEnumerable<HistoryRecord> ReadFirefox(string root, DateTimeOffset since)
    {
        ...
        foreach (var profile in Directory.EnumerateDirectories(root))
        {
            ...
            try
            {
                ...
                {
                    var micros = r.GetInt64(2);
                    yield return new HistoryRecord { Browser = "Firefox", Url = r.GetString(0), Title = r.IsDBNull(1) ? "" : r.GetString(1), VisitedUtc = DateTimeOffset.UnixEpoch.AddTicks(micros * 10) };
                }
            }
            catch { }
            finally { TryDelete(temp); }
    ...
    private static string? CopyDatabase(string source)
    {
        ...
        try { var target = Path.Combine(Path.GetTempPath(), $"superwall-{Guid.NewGuid():N}.db"); File.Copy(source, target, true); return target; }
        catch { return null; }
    }
