...
public sealed class SuperWallPolicy
{
    ...
    [JsonIgnore, Obsolete("Download approval now uses local Windows administrator authentication.")]
...
public sealed class InstalledApplicationsUpload
{
    public string DeviceId { get; set; } = "";
    public List<InstalledApplication> Applications { get; set; } = new();
}

public sealed class HistoryRecord { public string Browser { get; set; } = ""; public string Url { get; set; } = ""; public string Title { get; set; } = ""; public DateTimeOffset VisitedUtc { get; set; } }
public sealed class HistoryUpload { public string DeviceId { get; set; } = ""; public string RequestId { get; set; } = ""; public List<HistoryRecord> Records { get; set; } = new(); }
public sealed class AdminCommand { public string Id { get; set; } = Guid.NewGuid().ToString("N"); public string Type { get; set; } = ""; public DateTimeOffset CreatedUtc { get; set; } = DateTimeOffset.UtcNow; }
public sealed class PolicyEnvelope { public SuperWallPolicy Policy { get; set; } = new(); public List<AdminCommand> Commands { get; set; } = new(); }
public sealed class EnrollResponse { public string AgentToken { get; set; } = ""; public SuperWallPolicy Policy { get; set; } = new(); }
public sealed class EnrollmentKeyInfo { public string Id { get; set; } = ""; public string Label { get; set; } = ""; public DateTimeOffset CreatedUtc { get; set; } public DateTimeOffset ExpiresUtc { get; set; } public bool Used { get; set; } public bool Revoked { get; set; } public DateTimeOffset? UsedUtc { get; set; } }
public sealed class CreateEnrollmentKeyRequest { public string Label { get; set; } = ""; public int ExpiresMinutes { get; set; } = 30; }
public sealed class CreatedEnrollmentKey { public EnrollmentKeyInfo Info { get; set; } = new(); public string Key { get; set; } = ""; }