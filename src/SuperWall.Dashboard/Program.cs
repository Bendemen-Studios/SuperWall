...
bool IsAgent(HttpRequest r, string id)
{
    ...
    var stored = cmd.ExecuteScalar() as string; if (string.IsNullOrWhiteSpace(stored)) return false;
    return CryptographicOperations.FixedTimeEquals(Convert.FromHexString(stored), SHA256.HashData(Encoding.UTF8.GetBytes(value.ToString())));
}
...
{
    if (!IsAdmin(req)) return Results.Unauthorized(); using var c = new SqliteConnection($"Data Source={db}"); c.Open();
    using var check = c.CreateCommand(); check.CommandText = "SELECT COUNT(1) FROM commands WHERE device_id=$d AND type='request_history' AND completed=0"; check.Parameters.AddWithValue("$d", id);
    if (Convert.ToInt32(check.ExecuteScalar()) > 0) return Results.Conflict(new { error = "A history request is already pending for this device." });
...
{
    if (!IsAdmin(req)) return Results.Unauthorized(); using var c = new SqliteConnection($"Data Source={db}"); c.Open(); var policy = GetPolicy(c, id); using var cmd = c.CreateCommand(); cmd.CommandText = "SELECT browser,url,title,visited FROM history WHERE device_id=$d AND visited >= $from ORDER BY visited DESC LIMIT 5000"; cmd.Parameters.AddWithValue("$d", id); cmd.Parameters.AddWithValue("$from", DateTimeOffset.UtcNow.AddDays(-policy.SearchHistoryRetentionDays).ToString("O")); using var r = cmd.ExecuteReader(); var result = new List<HistoryRecord>(); while (r.Read()) result.Add(new HistoryRecord { Browser = r.GetString(0), Url = r.GetString(1), Title = r.GetString(2), VisitedUtc = DateTimeOffset.Parse(r.GetString(3)) }); return Results.Ok(result);
});
...
app.MapPost("/api/enroll/{id}", async (string id, HttpRequest req) =>
{
    ...
    using (var deviceCmd = c.CreateCommand()) { deviceCmd.Transaction = tx; deviceCmd.CommandText = @"INSERT INTO devices(device_id,name,os,profile,last_seen,agent_token_hash) VALUES($d,$n,$o,'Low',$t,$h) ON CONFLICT(device_id) DO UPDATE SET name=$n,os=$o,last_seen=$t,agent_token_hash=$h,profile='Low'"; deviceCmd.Parameters.AddWithValue("$d", id); deviceCmd.Parameters.AddWithValue("$n", body.GetValueOrDefault("computerName", id)); deviceCmd.Parameters.AddWithValue("$o", body.GetValueOrDefault("osVersion", "Windows")); deviceCmd.Parameters.AddWithValue("$t", now.ToString("O")); deviceCmd.Parameters.AddWithValue("$h", Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(token)))); deviceCmd.ExecuteNonQuery(); }
    tx.Commit(); using var c2 = new SqliteConnection($"Data Source={db}"); c2.Open(); return Results.Ok(new EnrollResponse { AgentToken = token, Policy = GetPolicy(c2, id) });
});

app.MapGet("/api/agent/{id}", (string id, HttpRequest req) =>
{
    if (!IsAgent(req, id)) return Results.Unauthorized(); using var c = new SqliteConnection($"Data Source={db}"); c.Open(); EnsureDevice(c, id); using var cmd = c.CreateCommand(); cmd.CommandText = "SELECT id,type FROM commands WHERE device_id=$d AND completed=0 ORDER BY created"; cmd.Parameters.AddWithValue("$d", id); using var r = cmd.ExecuteReader(); var commands = new List<AdminCommand>(); while (r.Read()) commands.Add(new AdminCommand { Id = r.GetString(0), Type = r.GetString(1) }); return Results.Ok(new PolicyEnvelope { Policy = GetPolicy(c, id), Commands = commands });
});

app.MapPost("/api/agent/{id}/history", async (string id, HttpRequest req) =>
{
    if (!IsAgent(req, id)) return Results.Unauthorized(); var upload = await JsonSerializer.DeserializeAsync<HistoryUpload>(req.Body); if (upload is null || !string.Equals(upload.DeviceId, id, StringComparison.Ordinal) || string.IsNullOrWhiteSpace(upload.RequestId)) return Results.BadRequest(); using var c = new SqliteConnection($"Data Source={db}"); c.Open(); var policy = GetPolicy(c, id); var cutoff = DateTimeOffset.UtcNow.AddDays(-policy.SearchHistoryRetentionDays); using var tx = c.BeginTransaction();
    using (var verify = c.CreateCommand()) { verify.Transaction = tx; verify.CommandText = "SELECT COUNT(1) FROM commands WHERE id=$id AND device_id=$d AND type='request_history' AND completed=0"; verify.Parameters.AddWithValue("$id", upload.RequestId); verify.Parameters.AddWithValue("$d", id); if (Convert.ToInt32(verify.ExecuteScalar()) != 1) { tx.Rollback(); return Results.Conflict(new { error = "History request is invalid, already completed, or not found." }); } }
    using var ins = c.CreateCommand(); ins.Transaction = tx; ins.CommandText = "INSERT INTO history(device_id,browser,url,title,visited) VALUES($d,$b,$u,$t,$v)"; foreach (var x in upload.Records.Where(x => x.VisitedUtc >= cutoff).Take(5000)) { ins.Parameters.Clear(); ins.Parameters.AddWithValue("$d", id); ins.Parameters.AddWithValue("$b", Truncate(x.Browser, 80)); ins.Parameters.AddWithValue("$u", Truncate(x.Url, 4096)); ins.Parameters.AddWithValue("$t", Truncate(x.Title, 512)); ins.Parameters.AddWithValue("$v", x.VisitedUtc.ToString("O")); ins.ExecuteNonQuery(); }
    using var complete = c.CreateCommand(); complete.Transaction = tx; complete.CommandText = "UPDATE commands SET completed=1 WHERE id=$id AND device_id=$d AND completed=0"; complete.Parameters.AddWithValue("$id", upload.RequestId); complete.Parameters.AddWithValue("$d", id); complete.ExecuteNonQuery(); tx.Commit(); return Results.Ok();
});
