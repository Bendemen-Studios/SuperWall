...
public static class ApplicationInventoryEndpoints
{
    public static void MapApplicationInventory(this WebApplication app, string db, Func<HttpRequest, bool> isAdmin, Func<HttpRequest, string, bool> isAgent)
    {
        app.MapGet("/api/devices/{id}/applications", (string id, HttpRequest req) =>
        {
            ...
            cmd.CommandText = "SELECT name,version,publisher,path,reported_utc FROM installed_apps WHERE device_id=$d ORDER BY name,version";
            cmd.Parameters.AddWithValue("$d", id);
            using var r = cmd.ExecuteReader();
            var result = new List<object>();
            while (r.Read())
                result.Add(new { name = r.GetString(0), version = r.GetString(1), publisher = r.GetString(2), path = r.GetString(3), reportedUtc = r.GetString(4) });
        ...
        app.MapPost("/api/agent/{id}/applications", async (string id, HttpRequest req) =>
        {
            ...
            InstalledApplicationsUpload? upload;
            try
            {
                upload = await JsonSerializer.DeserializeAsync<InstalledApplicationsUpload>(req.Body);
            }
            catch (JsonException)
            {
                return Results.BadRequest(new { error = "Invalid application inventory payload." });
            }

            if (upload is null || !string.Equals(upload.DeviceId, id, StringComparison.Ordinal)) return Results.BadRequest();

            var applications = (upload.Applications ?? new List<InstalledApplication>())
                .Take(2000)
                .Select(x => new
                {
                    Name = Truncate(x.Name, 256),
                    Version = Truncate(x.Version, 80),
                    Publisher = Truncate(x.Publisher, 256),
            ...
            {
                del.Transaction = tx;
                del.CommandText = "DELETE FROM installed_apps WHERE device_id=$d";
                del.Parameters.AddWithValue("$d", id);
                del.ExecuteNonQuery();
            }
            ...
            using (var ins = c.CreateCommand())
            {
                ...
                foreach (var x in applications)
                {
                    ...
                    ins.Parameters.AddWithValue("$n", x.Name);
                    ins.Parameters.AddWithValue("$v", x.Version);
                    ins.Parameters.AddWithValue("$p", x.Publisher);
                    ins.Parameters.AddWithValue("$x", x.Path);
            ...
            using (var clean = c.CreateCommand())
            {
                ...
                clean.CommandText = "UPDATE commands SET completed=1 WHERE device_id=$d AND type='request_app_inventory' AND completed=0";
                clean.Parameters.AddWithValue("$d", id);
                clean.ExecuteNonQuery();
            }

            tx.Commit();
            return Results.Ok(new { count = applications.Count });
        });
    }
