...
{
    public static void MapApplicationInventory(this WebApplication app, string db)
    {
        app.MapGet("/api/devices/{id}/applications", (string id, HttpRequest req) =>
        {
            var admin = req.HttpContext.RequestServices.GetService(typeof(object));
            if (!IsAdminRequest(req)) return Results.Unauthorized();
        ...
        {
            if (!IsAgentRequest(req, id, db)) return Results.Unauthorized();
            ...
            {
                ins.Transaction = tx; ins.CommandText = "INSERT INTO installed_apps(device_id,name,version,publisher,path,reported_utc) VALUES($d,$n,$v,$p,$x,$t)";
                foreach (var x in upload.Applications.Take(2000)) { ins.Parameters.Clear(); ins.Parameters.AddWithValue("$d",id); ins.Parameters.AddWithValue("$n",Truncate(x.Name,256)); ins.Parameters.AddWithValue("$v",Truncate(x.Version,80)); ins.Parameters.AddWithValue("$p",Truncate(x.Publisher,256)); ins.Parameters.AddWithValue("$x",Truncate(x.Path,4096)); ins.Parameters.AddWithValue("$t",DateTimeOffset.UtcNow.ToString("O")); ins.ExecuteNonQuery(); }
            }
    ...
    private static bool IsAdminRequest(HttpRequest req) => req.HttpContext.Items.ContainsKey("SuperWallAdminAuthenticated");
    private static bool IsAgentRequest(HttpRequest req,string id,string db)
    {
        if (!req.Headers.TryGetValue("X-SuperWall-Agent", out var value)) return false;
        using var c=new SqliteConnection($"Data Source={db}"); c.Open(); using var cmd=c.CreateCommand(); cmd.CommandText="SELECT agent_token_hash FROM devices WHERE device_id=$d"; cmd.Parameters.AddWithValue("$d",id); var stored=cmd.ExecuteScalar() as string;
        return !string.IsNullOrWhiteSpace(stored) && System.Security.Cryptography.CryptographicOperations.FixedTimeEquals(Convert.FromHexString(stored),System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(value.ToString())));
    }
}